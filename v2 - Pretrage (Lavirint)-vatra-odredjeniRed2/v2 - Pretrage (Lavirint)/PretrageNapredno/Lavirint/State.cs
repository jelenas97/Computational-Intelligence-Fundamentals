using System;
using System.Collections.Generic;
using System.Text;

namespace Lavirint
{
    public class State
    {
        public State parent;
        public Node node;
        public double cost;
        public int level;
        public bool kutijaBlue;
        public bool kutijaPink;


        public State sledeceStanje(Node node)
        {
            State rez = new State();
            rez.node = node;
            rez.parent = this;
            rez.cost = this.cost + 1;
            rez.level = this.level + 1;
            rez.kutijaBlue = this.kutijaBlue;
            rez.kutijaPink = this.kutijaPink;

            return rez;
        }


        public List<State> mogucaSledecaStanja()
        {

            if (Main.lavirint.polja[node.markI, node.markJ] == 4)
            {
                this.kutijaBlue = true;
            }

            if(Main.lavirint.polja[node.markI, node.markJ] == 6 && this.kutijaBlue)
            {
                this.kutijaPink = true;
            }

            List<State> rez = new List<State>();
            foreach (Node nextNode in this.node.getLinkedNodes())
            {
                rez.Add(this.sledeceStanje(nextNode));
            }
            return rez;
        }

        public bool isKrajnjeStanje()
        {
            return Main.krajnjiNode.Equals(this.node) && this.kutijaBlue && this.kutijaPink;
        }

        public List<State> path()
        {
            List<State> putanja = new List<State>();
            State tt = this;
            while (tt != null)
            {
                putanja.Insert(0, tt);
                tt = tt.parent;
            }
            return putanja;
        }

        public bool cirkularnaPutanja()
        {
            // TODO 3: proveriti da li trenutno stanje odgovara poziciji koja je vec vidjena u grani pretrazivanja
            
            return false;
        }

        public override int GetHashCode()
        {
            int hashValue = 0;
            if (this.kutijaBlue)
                hashValue += 1000;
            if (this.kutijaPink)
                hashValue += 10000;

            hashValue += 100 * node.markI + node.markJ;
            return hashValue;
        }
    }
}
